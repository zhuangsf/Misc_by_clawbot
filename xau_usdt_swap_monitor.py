#!/usr/bin/env python3
"""
XAU-USDT-SWAP 永续合约15分钟K线成交量监控程序
监控OKX黄金永续合约成交量，检测3倍放大异动
"""
...
try:
    ...
    USE_DB = True
    db_init()  # 确保数据库已初始化
    print("✅ XAU监控: 放量记录SQLite模块已加载")
except ImportError as e:
    USE_DB = False
    print(f"⚠️ XAU监控: 无法导入SQLite模块: {e}")
...
CONFIG = {
    ...
    # 日志配置
    "LOG_FILE": "/root/.openclaw/workspace/xau_swap_monitor.log",
    "VOLUME_LOG_FILE": "/root/.openclaw/workspace/xau_volume_spike_records.log",  # 放量记录文件
...
def setup_logging():
    ...
    logger = logging.getLogger('xau_swap_monitor')
    ...
    try:
        ...
        data = {
            'current': {
                ...
                'volume': float(current_kline[5]),  # 币种成交量（XAU）
            ...
            'previous': {
                ...
                'volume': float(previous_kline[5]),  # 币种成交量（XAU）
...
def send_feishu_alert(message):
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("✅ XAU永续合约警报已成功发送到飞书")
            STATE['alert_count'] += 1
            return True
        else:
            logger.error(f"❌ 发送XAU永续合约警报失败: {result.stderr}")
            return False
...
def check_volume_spike():
    ...
    if previous['volume_usdt'] > 0:
        ...
        if volume_ratio >= CONFIG['VOLUME_MULTIPLIER']:
            ...
            alert_message = f"""🚨 **XAU-USDT-SWAP 15分钟K线成交额异常！** ⚡️
...
def run_monitor():
    """运行监控程序"""
    logger.info("🚀 XAU-USDT-SWAP永续合约成交额监控程序启动！")
    logger.info("🎯 监控目标: OKX黄金永续合约 (XAUUSDT永续合约)")
