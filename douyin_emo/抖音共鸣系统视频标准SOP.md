...
**重要**：每行最多显示 **15个中文字符**，超过自动换行。

**换行规则**（`_create_ass_subtitle` 中的 `auto_wrap` 函数）：

| 优先级 | 规则 | 示例 |
|--------|------|------|
| 1️⃣ | **优先在符号处换行**（，。！？：；） | `但如果遇到了，` → `晚一点又有什么关系？` |
| 2️⃣ | 如果没有符号，超过15字在中间位置换行 | `这句话没有符号但很长很长` → `这句话没有符号但很长` + `很长` |

**示例**:
```
原文: "但如果遇到了，晚一点又有什么关系？"
换行后: "但如果遇到了，\N晚一点又有什么关系？"
